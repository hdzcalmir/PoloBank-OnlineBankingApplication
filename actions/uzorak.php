...
//             $statement16 = $db->prepare("SELECT imeuzorak FROM korisnici WHERE id_korisnika = ?"); 
//             $statement16->execute([$idprimaoca]); 
//             $rowsime1 = $statement16->fetchAll(); 
//             $imeprimaoca = '';
//             foreach ($rowsime1 as $row) { $imeprimaoca = $row['imeuzorak']; }

//                 if($racunuzorak != $racunposiljaoca) {
//                 $stmtmt6 = $db->prepare("INSERT INTO uzorci (id_korisnika, imeuzorak, racunuzorak, nazivuzorak, sumauzorak) VALUES (?, ?, ?, ?, ?)");
...
//                 $stmtmt6->execute([$idkorisnika, $username, $racunuzorak, $imeuzorka, $sumauzorak]);
//                 killConnection_PDO($db);
//                 echo'<script>window.location="../placanja.php";</script>';  
...
//                     }    
//                 }
//         elseif(empty($_POST['imeuzorak']) || empty($_POST['racunuzorak']) || empty($_POST['nazivuzorak']) || empty($_POST['sumauzorak'])){
//         $_SESSION['error'] = 'Molimo ispunite sva obavezna polja.';
//         killConnection_PDO($db);
//         echo'<script>window.location="../placanja.php";</script>';  
//         return true;
//         }    
